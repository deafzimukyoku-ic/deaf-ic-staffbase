...
/**
 * 社員側 シフト変更申請フォーム（復活: 旧 components/shift/ShiftChangeRequestForm.tsx を
 * git 5c274a9^ から復元し、施設シフトタブ MyFacilityShiftView から呼び出す）。
 *
 * - 申請種別: 時刻変更 / 休暇申請 / 勤務種別変更
 * - shift_change_requests に直接 INSERT（RLS scr_insert で本人 employee_id のみ書き込み可）
 * - 申請承認は admin の ApprovalQueueFull 経由（既存）
 * - 公休 (public_holiday) は管理者専用のため社員の申請選択肢に出さない
 */
...
interface Props {
  ...
  /** 本人の基本勤務時間（employees.default_start_time / default_end_time）。
   *  「希望の勤務時間」の初期値に使う（無ければ現状シフト → 09:00/17:00 にフォールバック）。 */
...
export default function ShiftChangeRequestForm({
  ...
  /* 初期値の優先順位: 本人の基本勤務時間 → 現状シフトの時刻 → 09:00/17:00 */
  const initStart = (defaultStartTime ?? currentShift?.start_time ?? '09:00').slice(0, 5);
  const initEnd = (defaultEndTime ?? currentShift?.end_time ?? '17:00').slice(0, 5);
  ...
  const [typeChangeStartTime, setTypeChangeStartTime] = useState((defaultStartTime ?? currentShift?.start_time)?.slice(0, 5) ?? '');
  const [typeChangeEndTime, setTypeChangeEndTime] = useState((defaultEndTime ?? currentShift?.end_time)?.slice(0, 5) ?? '');
  ...
  return (
    ...
    >
      <div className="space-y-4">
        {/* 現状表示 */}
        <div className="rounded-md bg-brand-beige p-3">
          <div className="text-xs font-bold text-brand-gray-light mb-1">現状のシフト</div>
          {currentShift ? (
            ...
            <div className="text-sm text-brand-gray">未設定</div>
          )}
        </div>

        {/* 申請種別選択 */}
        <div>
          <div className="text-xs font-bold text-brand-gray-light mb-1.5">申請の種類</div>
              ...
              <label
                ...
                className={`flex items-start gap-2 p-2 rounded-md border cursor-pointer transition ${
                  changeType === opt.value
                    ...
                    : 'border-brand-gray/15 hover:bg-brand-blue/5'
                }`}
              >
                <input
                  type="radio"
                ...
                <div>
                  ...
                  <div className="text-xs text-brand-gray">{opt.description}</div>
                </div>
              </label>
            ))}
          </div>
        ...
        {/* 詳細入力 */}
        <div className="rounded-md bg-white border border-brand-gray/15 p-3 space-y-3">
          {changeType === 'time' && (
            <div>
              <div className="text-xs font-bold text-brand-gray-light mb-1.5">希望の勤務時間</div>
              <div className="flex items-center gap-2">
                <input
                  type="time"
                  value={startTime}
                  onChange={(e) => setStartTime(e.target.value)}
                  className="text-sm rounded-md px-2 py-1 border border-brand-gray/20 outline-none focus:border-brand-blue/40"
                />
          ...
          {changeType === 'leave' && (
            <div>
              <div className="text-xs font-bold text-brand-gray-light mb-1.5">休暇種別</div>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm rounded-md border transition ${
                      leaveType === t
                        ...
                        : 'border-brand-gray/20 hover:bg-brand-blue/5'
                    }`}
                  >
                    {ASSIGNMENT_LABELS[t]}
                  </button>
          ...
          {changeType === 'type_change' && (
            <>
              <div>
                <div className="text-xs font-bold text-brand-gray-light mb-1.5">変更後の種別</div>
              ...
              {typeChangeType === 'normal' && (
                <div>
                  <div className="text-xs font-bold text-brand-gray-light mb-1.5">勤務時間</div>
                  <div className="flex items-center gap-2">
                    <input
                      type="time"
                      value={typeChangeStartTime}
                      onChange={(e) => setTypeChangeStartTime(e.target.value)}
                      className="text-sm rounded-md px-2 py-1 border border-brand-gray/20"
                    />
                    <span className="text-brand-gray">〜</span>
                    <input
                      type="time"
        ...
        {/* 理由 */}
        <div>
          <label className="text-xs font-bold text-brand-gray-light block mb-1">理由（任意）</label>
          <textarea
            value={reason}
            onChange={(e) => setReason(e.target.value.slice(0, 200))}
            rows={2}
            placeholder="管理者に共有する理由があれば記入してください"
            className="w-full text-sm rounded-md px-3 py-2 border border-brand-gray/15 outline-none focus:border-brand-blue/40"
          />
        </div>
