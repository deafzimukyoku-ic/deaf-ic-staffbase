-- 183: get_my_subordinate_progress に audience 判定用カラム追加
--
-- 背景:
-- mgr/dashboard で部下の達成率を出すとき、分母 (4 機能の対象件数) は per-employee で
-- 計算する必要がある (他施設限定や position 限定アイテムは「対象外社員」の分母から除外したい)。
-- ただし manager は subordinate の employees.position_id や employee_facilities を直接
-- SELECT する RLS 権限が無いため、ここで RPC を SECURITY DEFINER 経由で渡す。
--
-- 変更内容:
-- 156 で作った RPC の RETURNS TABLE に
--   position_id              uuid              — employees.position_id
--   additional_facility_ids  uuid[]            — employee_facilities (兼任先) を array_agg
-- を末尾追加 (後方互換: 既存呼び出し側は追加列を無視するだけ)。
--
-- 追加された 2 列を client 側 (admin/mgr dashboard) で使い、isItemInAudience() で
-- 4 機能 (compliance/training/announcement/manual) の publishedTotals を per-employee 算出する。

-- PostgreSQL は RETURNS TABLE の構造変更時に CREATE OR REPLACE では更新できないため、
-- 先に DROP してから CREATE する (CASCADE 不要: 他の関数からの依存なし)。
DROP FUNCTION IF EXISTS public.get_my_subordinate_progress(uuid);

CREATE FUNCTION public.get_my_subordinate_progress(p_facility_id uuid DEFAULT NULL)
RETURNS TABLE (
  employee_id uuid,
  tenant_id uuid,
  last_name text,
...
COMMENT ON FUNCTION public.get_my_subordinate_progress(uuid) IS
  'admin / manager / shift_manager 用 部下進捗集計 RPC。156 + 183 で position_id と additional_facility_ids[] を末尾追加。client 側で audience 判定 (isItemInAudience) に使う。';
