// シフト・送迎ダッシュボード（admin）
// ShiftPuzzle の dashboard を模倣：ウェルカム + 6枚カードグリッド（完成/未完成バッジ）
...
const ADMIN_CARDS: DashboardCard[] = [
  /* 業務（左→右、上→下の作業順）*/
  { href: '/admin/shifts/schedule', title: '利用表', desc: 'PDFインポート・カレンダー確認', icon: '📅', key: 'schedule' },
  { href: '/admin/shifts', title: 'シフト表', desc: 'シフト生成・調整・公開', icon: '📋', key: 'shift' },
  { href: '/admin/shifts/transport', title: '送迎表', desc: '担当割り当て・公開', icon: '🚗', key: 'transport' },
  /* 出力 */
  { href: '/admin/shifts/output/daily', title: '日次出力', desc: '当日の送迎・出勤をホワイトボード風に表示', icon: '📄' },
  { href: '/admin/shifts/output/daily-report', title: '業務日報', desc: '当日の業務内容を A4 で出力', icon: '📋' },
  { href: '/admin/shifts/output/billing', title: '利用料金表', desc: '月次の請求書を生成・印刷', icon: '💰' },
  /* 申請 */
  { href: '/admin/requests', title: '休み希望一覧', desc: '全職員の提出状況を確認', icon: '✋', key: 'request' },
...
async function computeStatuses(tenantId: string, targetMonthStr: string, monthFrom: string, monthTo: string): Promise<Record<CardKey, Status>> {
...
export default async function AdminShiftsDashboardPage() {
  ...
  const statuses = tenantId
    ? await computeStatuses(tenantId, targetMonthStr, monthFrom, monthTo)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* カードグリッド（shift_only_mode 対応はクライアント側で実施） */}
      <DashboardCardsGrid cards={ADMIN_CARDS} statuses={statuses} scope="admin" />
    </div>
  );
}
