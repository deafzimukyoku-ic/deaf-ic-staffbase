...
---
## 郵便番号の末尾1桁が消える（住所自動補完が古い state を書き戻していた）

- **発生日**: 2026-08-09（ユーザー報告「郵便番号の保存時？に一桁足りなくなっているかもしれません」）
- **発生箇所**: `components/employee/ProfileSection1Basic.tsx` / `ProfileSectionContacts.tsx` の `update()`
- **エラー内容**: 例外なし。社員プロフィールの郵便番号が `451-0072` → `451-007` のように
  **末尾1桁だけ欠けて保存される**。住所は正しく入っている。
- **原因**: 典型的な stale closure。

  ```js
  function update(key, value) {
    onChange({ ...data, [key]: value });   // data はレンダー時にキャプチャされた props
  }
  ```

  7桁目を入力した瞬間の流れ:
  1. `onChange('451-0072')` → 親の state は正しく更新される
  2. 7桁揃ったので `PostalCodeField` が住所検索の fetch を非同期で開始
  3. 数百ms後に応答 → `onAddressFound(住所)` が**手順1と同じレンダーの closure から**呼ばれる
  4. `onChange({ ...古いdata, address: 住所 })` で **postal_code が1文字前の状態に巻き戻る**

- **調査で潰した仮説（記録）**:
  - DB 列の長さ制限 → `text`（無制限）。**違う**
  - `maxLength={8}` による切り詰め → git 履歴上ずっと 8 で、7桁入力は 8 文字に収まる。**違う**
  - 保存 API / slice の切り詰め → `slice(0, 7)` は digits に対してで正しい。**違う**
  - 一括インポート時の事故 → created/updated が数ヶ月に分散。**違う**
  - 「利用者が6桁しか入力していない」→ **異常な23件すべてに住所が入っていた**。
    住所補完は7桁揃わないと発火しないので、7桁打てた人だけが壊れている。これが決め手になった
- **書式の内訳が仮説と完全一致**: 6桁ベタ17件（数字だけ打った人の巻き戻り先）/
  3桁-3桁 6件（ハイフンを自分で打った人の巻き戻り先）。整形は7桁到達時にしか起きないので、
  「3桁-3桁」は入力では作れない＝巻き戻りの証拠。
- **解決方法**:
  - 両セクションの `update()` を「最新の data を指す ref」に対してマージするよう変更。
    ref の同期はレンダー中ではなく `useEffect` で行う（レンダー中の ref 書き込みは React の規約違反）
  - `PostalCodeField` に桁数不足の警告を追加（1〜6桁のときだけ「7桁です（現在N桁）」を表示）
  - 既存データは `scripts/repair-truncated-postal-codes.mjs` で復旧。**47件すべて自動特定して修正**
- **復旧の方法（再利用可能）**: 末尾1桁は失われているが住所は残っているので、
  6桁 + 0〜9 の10通りを郵便番号APIに問い合わせ、保存住所と前方一致する候補を探す。
  精度を出すのに次の2点が必要だった:
  1. **町域が空の「以下に掲載がない場合」コードは弱い一致として区別する**。
     市区町村までしか一致せずほぼ何にでも当たるため、同列に扱うと候補が絞れない
  2. **町域の最長一致を採る**。「矢田」は「矢田南」の前方一致になるので、
     より具体的な町域を選ばないと2候補で止まる
  3. 保存住所は都道府県が省略されていることがあるので、**都道府県あり／なしの両方**で照合する
- **再発防止**:
  - **非同期コールバックから呼ばれる更新関数で `{...props.data}` を展開しない。**
    fetch の応答が返る頃には、その data はもう古い。ref か関数型更新を使う
  - 同じ `onChange({ ...data, [key]: value })` パターンは他のプロフィール節にもあるが、
    **非同期コールバックを持つのは PostalCodeField を使う2ファイルだけ**なので、そこだけ直した
  - 入力欄は「不正な値を黙って受け取らない」。桁数が決まっている項目は未達を明示する
- **関連**: `scripts/repair-truncated-postal-codes.mjs` / `components/employee/PostalCodeField.tsx`
---
*(以降、新規エラーがあれば追記)*
