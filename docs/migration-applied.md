...
| 225 | `225_sibling_pick_children_rpc.sql` | 2026-08-08 | 2han2be4han@gmail.com | `scripts/apply-migration-225.mjs`（兄弟の設定を「グループを作る」→「**他の児童を選ぶ**」へ。職員にグループ名を考えさせない。`sibling_groups.label` を nullable 化（グループは内部表現として残す＝請求担当事業所の置き場所）+ SECURITY DEFINER RPC 2 本を追加。<br>`get_sibling_candidates()`: 選択候補をテナント全域で返す。**返すのは id / 氏名 / 事業所 / 現在のグループのみ**で住所・連絡先・上限額・請求額は返さない。employee は 0 件。<br>`set_child_siblings(uuid, uuid[])`: きょうだいの紐付け。**別事業所の児童の `sibling_group_id` も更新する必要がある**（兄弟が別事業所に通うケース）が、children の RLS を緩めると請求額まで見えるため、**触る列を `sibling_group_id` だけに限定した RPC** に閉じ込めた（migration 214 と同じ方針）。自テナント検証あり。1 人以下になったグループは自動削除。<br>apply 時に実ロールの JWT 偽装で 候補 73 名 / 3 事業所 取得・別事業所どうしの紐付け成立・解除でグループ自動片付け・employee は候補 0 件かつ紐付け例外 を実証。children / billing 系 4 テーブルの policy 述語は適用前後で完全一致（事業所分離は緩めていない）。storage 非変更のため snapshot 不要） |
