...
| 180 | 180_notification_queue_first_scheduled.sql | **通知メールのローリングウィンドウ集約基盤**。`notification_queue.first_scheduled_at timestamptz NOT NULL` 追加 (既存 pending 行は scheduled_at で backfill)。24h 以上 overdue な未送信 pending は `cancelled_at=now()` で破棄。enqueue / cron 抽出最適化用の部分 INDEX `idx_notif_queue_pending_scheduled (tenant_id, scheduled_at) WHERE pending` を追加。`app/api/notifications/enqueue/route.ts` がこれを使って同テナント未送信行を rolling window で揃え、最古行の `first_scheduled_at + MAX_DELAY_HOURS=6` を hardCap として強制送信する | ✅ 適用済 |
| 181 | 181_pg_cron_notification_dispatch.sql | **GitHub Actions cron 廃止 → Supabase pg_cron 移行**。GH Actions schedule は best-effort 仕様で discard 多発 (5/17 0 件, 5/15→5/18 で 3 件しか発火しない事例)、Vercel Hobby は cron 1 日 1 回のみ、という制約下で Supabase 内完結方式を採用。`pg_cron` + `pg_net` 拡張を有効化し、`*/10 * * * *` で `net.http_post(url := vault('cron_target_url') || '/api/cron/send-notifications', Authorization: Bearer vault('cron_secret'))` を実行する `dispatch_notification_queue` ジョブを作成。Vault に `cron_target_url` / `cron_secret` を別途登録する前提 | ✅ 適用済 |
| 200 | 200_push_subscriptions.sql | **PWA Web Push 用 subscription テーブル**。`push_subscriptions (id, employee_id, endpoint UNIQUE, p256dh, auth, user_agent, created_at, last_used_at, last_failed_at)` + RLS 4 ポリシー (本人 SELECT/INSERT/UPDATE/DELETE)。配信は service role 経由で `lib/push/server.ts:sendWebPushToEmployees` (web-push 並列 25 件、410 Gone で自動削除)。`docs/features/pwa-push-notifications.md` 仕様準拠 | ✅ 適用済 |
| 210 | 210_documents_rls_active_only.sql | **documents バケット SELECT/ALL RLS に `status='active'` 条件を追加**。退職者 (`employees.status='retired'`) の URL 直アクセスを構造遮断。docs/features/content-media-signed-url.md Phase 1 | ✅ 適用済 |
...
| **supabase/migrations/222_billing_fee_items.sql** | `billing_fee_items`（請求項目マスタ）/ `children_fee_amounts`（児童別月額）/ `billing_summary_fee_amounts`（月次スナップショット）新規 + RLS 6 本。**`billing_summary_fee_amounts.fee_item_id` は `on delete restrict`**（スナップショットを持つ項目は削除不可＝過去月の紙を守る）。`system_key` に部分 UNIQUE（施設ごとに 1 つ＝移行の冪等性） |
| **supabase/migrations/223_children_sibling_group.sql** | `sibling_groups` 新規 + `children.sibling_group_id`（`on delete set null`）+ RLS 2 本。兄弟の小計行用 |
| **supabase/migrations/225_sibling_pick_children_rpc.sql** | 兄弟の設定を「グループ作成」→「**他の児童を選ぶ**」へ。`label` を nullable 化（グループは請求担当の置き場所として内部に残す）+ RPC `get_sibling_candidates()`（候補一覧・テナント全域・氏名と事業所のみ）/ `set_child_siblings(uuid, uuid[])`（紐付け・触るのは `sibling_group_id` のみ・1人以下のグループは自動削除）。**別事業所の児童を更新する必要があるため SECURITY DEFINER**（children の RLS は緩めない）|
| **supabase/migrations/224_sibling_groups_tenant_scope.sql** | 兄弟グループを**法人全体スコープ**化（UNIQUE を `(tenant_id, label)` に張り替え / RLS をテナントスコープ `sg_manager_tenant` に置換）+ `billing_facility_id`（請求担当事業所）追加。**`children` / `billing_*` の RLS は不変＝事業所分離は緩めない**。「きょうだい合計は自事業所の児童のみ」という決定に基づく |
| **scripts/migrate-billing-fee-items.mjs** | データ移行（dry-run 既定 / `--apply` / 冪等）。組込項目シード + `kumon_monthly_fee` → `children_fee_amounts` + `billing_summaries` → `billing_summary_fee_amounts`。**移行前後の金額を実データで突合し一致しなければ ROLLBACK** |
| lib/types.ts | `CopayTier` / `EventRow` / **`FeeCalcType` / `BillingFeeItemRow` / `ChildFeeAmountRow` / `BillingSummaryFeeAmountRow` / `SiblingGroupRow`** 追加、`ChildRow` 拡張（`sibling_group_id` 追加 / `kumon_monthly_fee` は @deprecated） |
| lib/constants.ts | `SNACK_FEE_PER_DAY=50` / `COPAY_TIERS` / `COPAY_TIER_AMOUNT` / `COPAY_TIER_LABELS` / `NAGOYA_FREE_PRESCHOOL_MUNICIPALITY` / `FREE_GRADES_NATIONWIDE`<br>※ **migration 222 以降、`SNACK_FEE_PER_DAY` はアプリの計算では使われない**。役割は移行スクリプトが組込項目「おやつ等」をシードするときの既定単価のみ。実際の単価は `billing_fee_items.unit_amount`（請求項目設定で変更可）が正 |
| lib/logic/computeBilling.ts | `isFreeOfCharge` / `resolveCopayCap` / `computeDefaultCopayAmount` / **`computeDefaultFeeAmount` / `resolveFeeAmount` / `stepFeeAmount` / `isFeeOverridable` / `computeSiblingSubtotals` / `EMPTY_FEE_VALUE`** / `computeBillingRow(child, days, feeItems, feeValues, events, copayOverride?)`（純関数）<br>※ 旧 `computeDefaultSnackFee` / `resolveSnackFee` / `stepSnackFee` は `per_day` の特例に過ぎなくなったため **`resolveFeeAmount` / `stepFeeAmount` に統合して削除**（式の二重定義を作らない）<br>※ `amountOverride` の `0`（手動0円）と `null`（自動）は別物。判定は `== null` で行い `\|\|` を使わない<br>※ **`checkbox` は OFF なら override が残っていても必ず 0**（誤課金の防止） |
| components/shift/BillingItemSettingsFull.tsx | 請求項目設定 ページ（CRUD / 並び替え / 有効 OFF / 削除ガード）。組込項目（`system_key` あり）は**計算方式を変更不可**（過去月の意味が変わるため）。スナップショットを持つ項目は削除不可で理由を表示 |
| app/(admin)/admin/shifts/billing-items/, app/(manager)/mgr/shifts/billing-items/ | BillingItemSettingsFull のページラッパ。`/admin/shifts` 配下なので shift_manager も到達可（middleware の許可条件） |
| components/shift/ChildrenSettingsFull.tsx | 児童編集モーダルに料金属性 UI、児童一覧テーブルに「上限 / 月額項目 / 兄弟」列。**`per_child_monthly` 型の請求項目ぶんの金額欄を動的生成**（旧「教材印刷代」固定欄を置換）+ 兄弟グループの選択・新規作成。保存時に `children_fee_amounts` を upsert（0 / 空欄は行ごと delete）し、旧列 `kumon_monthly_fee` は `system_key='material'` の額と同期して書き続ける |
| components/shift/EventSettingsFull.tsx | イベント設定 ページ（CRUD + 月切替）|
| components/shift/BillingFull.tsx | 利用料金表 ページ（月選択 / 自動計算 / 手動オーバーライド / おやつ等の ▲▼ 調整 / 保存 / A4 横印刷 / Excel 出力）。`RowState.snackOverride` ⇄ `billing_summaries.snack_fee_override`<br>**イベント参加チェックの初期値規則（2026-08-08 / `billing-event-check-follow.md`）**: `billing_event_participations` に**行があれば** `false` 含めその保存値を使う（職員の明示判断として尊重）。**行が無ければ**（未保存の月、または保存後に追加されたイベント）利用表の出席実績 `isAttended` から初期化する。判定は `partsMap.get(id) ?? false` ではなく **`partsMap.has(id)`**（`??` だと出席していても永久に OFF になる。実 DB で 52 件検出）。保存値は自動で書き換えず、出席実績とのズレは `RowState.attendedByEvent` と突き合わせて ⚠/＋ で提示し、「出席実績に合わせる」を押したときだけ揃える<br>**列順（先方要望 2026-08-08）**: `# / 市町村 / 氏名 / 出席日数 / 利用負担額 / 【常時かかる項目】/ 各イベント / 参加費合計 / 【チェック型の項目】/ 請求額 / 兄弟`。チェック型（他施設利用など）は「その月だけ発生する追加費用」なので参加費合計の右へ、兄弟は最終列。`mainFeeItems`（非 checkbox）/ `checkboxFeeItems` に分けて描画し、セルと見出しは `renderFeeCell` / `renderFeeHeader` に一本化する（左右 2 箇所に書くとズレるため）。`totalCols = 5 + 請求項目数 + イベント数 + 3`<br>**請求項目の動的列（2026-08-08 / migration 222）**: 列は `billing_fee_items`（`is_active=true`）から `display_order` 順に生成。`per_day`/`monthly_fixed` は ▲▼+↺、`checkbox` はチェック、`per_child_monthly` は読み取り専用。値は `RowState.feeValues` ⇄ `billing_summary_fee_amounts`。保存は **upsert（delete しない）**＝無効化した項目の過去スナップショットを消さないため<br>**兄弟小計（migration 223 / 224）**: `orderedRows` で同一グループを隣接させ（未所属児の順序は不変）、グループ最終行の直下に「きょうだい合計」行を挿入。**全体合計は `rows`（児童行）のみを集計**し小計行を足さない＝二重計上の防止。グループは法人全体スコープなので `sibling_groups` は facility で絞らず取得する。**きょうだい合計は自事業所の児童のみ**（`rows` 自体が自事業所スコープなので自動的にそうなる）で、他事業所の金額は取得も表示もしない。`billing_facility_id` が他事業所なら小計行に「⚠ 請求は◯◯が担当」を出して二重請求を防ぐ |
